	...
	{% if product.images_count > 0 %}
		<div class="col-md {% if not has_multiple_slides %}product-with-one-image pr-md-3{% else %}p-0 px-md-3{% endif %}">
			<div class="js-swiper-product swiper-container product-detail-slider" style="visibility:hidden; height:0;" data-product-images-amount="{{ product.images_count }}">
				...
				<div class="swiper-wrapper">
					{% for image in product.images %}
						{% set variation_image = false %}

						{% for variation in product.variations %}
							{% if product.default_options[variation.id] != "" %}

								{% set actual_variant = product.variants_object[variation.id] %}

								{% if actual_variant.image == image.id and loop.index0 > 0 %}
									{% set variation_image = true %}
								{% endif %}
							{% endif %}
						{% endfor %}

						<div class="js-product-slide w-100 swiper-slide product-slide{% if home_main_product %}-small{% endif %} slider-slide{% if variation_image %} variant-selected{% endif %}" data-image="{{image.id}}" data-image-position="{{loop.index0}}">
							{% if home_main_product %}
