...
DOMContentLoaded.addEventOrExecute(() => {
	...
	{% if template == 'home' %}
        ...
        {% if has_featured_products_slider or has_new_products_slider or has_sale_products_slider or has_promotion_products_slider or has_best_seller_products_slider %}
	...
	{% if template == 'category' %}
        ...
        var hideProd = function() {
            let products = document.querySelectorAll('div[id^="product_color_"]');

            if(!products) return;

            products = [...products];

            if(prodsChange != products.length)
            {
                ...
                if (searchParams.has('Color')) 
                {
                    ...
                    [...products].forEach(element => {
                        ...
                        let checkColor = color_variant_selected.find(color => {
                            let id_product_hide = 'product_color_' + color;

                            return element.id.toUpperCase() == id_product_hide.toUpperCase()
                        })

                        if(!checkColor) element.remove();
                    });
                }
            }
        }
    ...
    {% if template == "product" %}
        ...
        {
            setTimeout(() => {
                let selected = document.querySelector(".js-insta-variant.color.selected")
        ...
        setTimeout(() => {
            try {
                const actualVariant = LS.variants.filter(v => v.id == LS.urlParams.variant).pop();

                if(!actualVariant) return;
    
                const slide = document.querySelector('.js-product-slide[data-image="'+actualVariant.image+'"]');
    
                document.querySelector('.js-swiper-product').swiper.slideTo(slide.dataset.imagePosition);
            }
            catch(e) {
                console.log(e)
            }
    ...
    {% if template == '404' and show_help %}
        ...
        const productSilder = createSwiper('.js-swiper-product', {
            lazy: true,
            slidesPerView: 'auto',
            watchOverflow: true,
            navigation: {
        ...
        let variantSlide = document.querySelector(".js-product-slide.variant-selected");

        if(variantSlide)
        {
            const index = variantSlide.dataset.imagePosition;
            console.log(index);
            document.querySelector('.js-swiper-product').swiper.slideTo(index)
        }
